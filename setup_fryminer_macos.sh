...
EOF
    ...
    if [ "$IS_UNMINEABLE" = "true" ]; then
        cat >> "$SCRIPT_FILE" <<EOF
        $HOME/.fryminer/miners/xmrig -o $POOL -u \$USER_WALLET${WORKER:+.$WORKER}#$UNMINEABLE_REFERRAL -p \$USER_PASSWORD --threads=$THREADS -a $ALGO --no-color --donate-level=0 $XMRIG_OPTS 2>&1 | tee -a "\$LOG" &
        CPU_PID=\$!
EOF
    else
        cat >> "$SCRIPT_FILE" <<EOF
